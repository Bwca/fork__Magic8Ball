...
const main = async () => {
  ...
  let tex = await textureLoader.loadAsync(
  ...
  let g1 = new THREE.SphereGeometry(
    4,
    200,
    100,
    0,
  ...
  let g4 = new THREE.SphereGeometry(
    3.99,
    200,
    25,
    0,
  ...
  ig.setAttribute(
    "instId",
    new THREE.InstancedBufferAttribute(new Float32Array([0, 1, 2, 3]), 1)
  );
  ...
  let step = 0.05;
  iWriting.setMatrixAt(
    0,
    new THREE.Matrix4().setPosition(0, 0.75 * 4 - step * 3, 0)
  );
  iWriting.setMatrixAt(
    1,
    new THREE.Matrix4().setPosition(0, 0.75 * 4 - step * 2, 0)
  );
  iWriting.setMatrixAt(
    2,
    new THREE.Matrix4().setPosition(0, 0.75 * 4 - step * 1, 0)
  );
  iWriting.setMatrixAt(
    3,
    new THREE.Matrix4().setPosition(0, 0.75 * 4 - step * 0, 0)
  );
    ...
    [
      ...
      phraseChunks.forEach((pc, idx) => {
        ctx.fillText(
          pc.toUpperCase(),
          127,
          127 - startPoint + idx * size * sizeRatio
        );
      });
